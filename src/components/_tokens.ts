...
const buildTintsAndShades = (baseColors, tintsAndShades) => {
  ...
  Object.keys(baseColors).forEach((colorName) => {
    const colorCode = baseColors[colorName];

    tintAndShadeNames.forEach((tintOrShadeName) => {
      const tintOrShader = tintsAndShades[tintOrShadeName];

      expandedColors[`${colorName}--${tintOrShadeName}`] = tintOrShader(colorCode);
    });
  });
