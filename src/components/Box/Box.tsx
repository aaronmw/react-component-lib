...
import { RESPONSIVE_BREAKPOINTS } from '../_tokens';
...
export type BoxProps<Component extends React.ElementType = 'div'> = CoreBoxProps<Component> &
  Omit<React.ComponentPropsWithRef<Component>, keyof CoreBoxProps<Component> | keyof BoxStyleProps>;

type BoxComponent = <Component extends React.ElementType = 'div'>(props: BoxProps<Component>) => JSX.Element;
...
const Box: BoxComponent = styled.div
  .attrs((props) => ({
    ...
    'aria-hidden': (props as any).isOnlyForScreenReaders === true ? 'true' : null,

    // FontAwesome forces area-hidden to "true" no matter what; this
    // gives us a selector for explicitly hidden Boxes
    'data-is-hidden': (props as any).isOnlyForScreenReaders === true ? 'true' : null,
  }))
  .withConfig({
    shouldForwardProp: (propName, nativeFilter) => {
      return (
        alwaysForwardedProps.includes(propName as any) ||
        (boxPropNames.includes(propName as any) === false && nativeFilter(propName) === true)
      );
    },
