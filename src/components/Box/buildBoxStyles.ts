...
import { BoxStyleProps, Color } from './Box.types';

// Exported for tests
export const CHILDREN_SELECTOR = '& > *:not([data-is-hidden]) + *:not([data-is-hidden])';

const toBorder = (borderPropName, props) => {
  const propValue = props[borderPropName];
  const borderColor = props[`${borderPropName}Color`] || props['borderColor'] || 'border';
  const borderColorLightness = props[`${borderPropName}ColorLightness`] || props[`borderColorLightness`] || undefined;

  return propValue === false
    ? 'none'
    : `${BORDER_STYLES[propValue === true ? 'normal' : (propValue as string)]} ${String(
        toColor(borderColor, borderColorLightness),
      )}`;
};

const toBorderColor = (borderPropName, borderColorPropertyName) => (borderColorPropValue, props) => ({
  ...(!props[borderPropName] && {
    [borderColorPropertyName]: toColor(borderColorPropValue, props[`${borderPropName}ColorLightness`]),
  }),
});

const toBorderRadius = (borderRadiusPropValue) => BORDER_RADII[borderRadiusPropValue];

const toColor: (colorName: Color, adjustment?: boolean | string | number) => Color = (
  colorName,
  adjustment = false,
) => {
  ...
  const baseColorName = colorName in SPOT_COLORS ? SPOT_COLORS[colorName] : colorName;
  ...
  const adjustedLightnessValue = clamp(
    typeof adjustment === 'string' ? Number(currentLightnessValue) + Number(adjustment) : (adjustment as number),
    100,
    700,
  );
...
const toLength = (propValue) => (typeof propValue === 'number' ? `${propValue}px` : propValue);
...
const boxPropRenderers: Array<BoxPropRenderer> = [
  {
    ...
    getCSS: (_, { as, disabled, highlightFocusWithin, href, position, tabIndex, zIndex, onClick }) =>
      ...
      typeof tabIndex === 'number'
        ? {
            ...
            [highlightFocusWithin ? '&:focus-within' : '&:focus']: [
              ...
              `z-index: ${typeof zIndex === 'number' ? zIndex : Z_INDEXES[zIndex || '1--stickyElements']};`,
            ].join('\n'),
          }
        : {},
  },
  ...
  },
  {
    ...
    getValue: (backgroundColor, { backgroundColorLightness }) => toColor(backgroundColor, backgroundColorLightness),
  },
  {
    propName: 'border',
    cssProperty: 'border',
  ...
  },
  {
    ...
    getCSS: toBorderColor('borderBottom', 'border-bottom-color'),
  },
  {
    propName: 'borderLeft',
  ...
  },
  {
    ...
    getCSS: toBorderColor('borderLeft', 'border-left-color'),
  },
  {
  ...
  },
  {
    ...
    getCSS: toBorderColor('borderRight', 'border-right-color'),
  },
  {
    propName: 'borderTop',
  ...
  },
  {
    ...
    getCSS: toBorderColor('borderTop', 'border-top-color'),
  },
  {
    propName: 'bottom',
    cssProperty: 'bottom',
  ...
  },
  {
    ...
    getCSS: (columnGap, { columns }) =>
      columns
        ...
        : {
            display: 'flex',
            [CHILDREN_SELECTOR]: `margin-left: ${toGridSizeOrLength(columnGap)};`,
          },
  },
  {
    propName: 'columns',
    getCSS: (propValue) => {
      const cssPropertyValue =
        typeof propValue === 'number' // 3 => '1fr 1fr 1fr'
          ...
          ? propValue.map((column) => (typeof column === 'number' ? `repeat(${column}, 1fr)` : column)).join(' ')
          : propValue;
  ...
  },
  {
    ...
    getCSS: (gap, { columns, flexDirection }) => {
      ...
      if (columns === undefined) {
        ...
        styles['display'] = 'flex';
        styles[CHILDREN_SELECTOR] = `margin-${marginEdgeByFlexDirection[flexDirection || 'row']}: ${toGridSizeOrLength(
          gap,
        )};`;
      } else {
  ...
  },
  {
    ...
    getCSS: (responsiveProps) => {
      ...
      breakpointNames.forEach((breakpointName) => {
        const breakpointSelector = RESPONSIVE_BREAKPOINTS[breakpointName];
    ...
    getCSS: (rowGap, { columns, flexDirection }) =>
      columns
        ...
        : {
            ...
            [CHILDREN_SELECTOR]: `margin-top: ${toGridSizeOrLength(rowGap)};`,
          },
  },
  {
    propName: 'textAlign',
  ...
  },
  {
    propName: 'zIndex',
    cssProperty: 'z-index',
    getValue: (zIndex) => (typeof zIndex === 'number' ? zIndex : Z_INDEXES[zIndex]),
  },
];
...
const buildBoxStyles = (props: BoxStyleProps = {}) => {
  ...
  boxPropRenderers.forEach((boxPropRenderer) => {
    ...
    if (([] as Array<string>).concat(propName).every((v) => [null, undefined].includes(props[v]))) {
      return;
    }
  ...
  const cssForNestedSelectors = Object.keys(nestedSelectors)
    .sort()
    .map((selector) => [`${selector} {`, nestedSelectors[selector], '}'].join('\n'));
